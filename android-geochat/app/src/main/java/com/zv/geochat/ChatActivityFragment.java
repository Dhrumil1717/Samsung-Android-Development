...
public class ChatActivityFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Button btnSendRandomId = (Button) v.findViewById(R.id.btnRandomId);
        btnSendRandomId.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view)
            {
                Random r = new Random();
                Integer randomID = r.nextInt(10000000+1);
                String convertedID = String.valueOf(randomID);
                sendRandomID(convertedID);
    ...
    private void sendRandomID(String convertedID)
    {
        Bundle data = new Bundle();
         data.putInt(ChatService.MSG_CMD, ChatService.SEND_RANDOM_ID);
        data.putString(ChatService.KEY_MESSAGE_TEXT,convertedID);
        Intent intent = new Intent(getContext(), ChatService.class);
        intent.putExtras(data);
        getActivity().startService(intent);
    }
    ...
    private void sendError(){
        Bundle data = new Bundle();
        data.putInt(ChatService.MSG_CMD, ChatService.CONNECT_ERROR_91);
        Intent intent = new Intent(getContext(), ChatService.class);
        intent.putExtras(data);
        getActivity().startService(intent);
    }
